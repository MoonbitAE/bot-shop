/**
 * Bot Simulation for AI-cessible Airline Shopping
 * 
 * This script simulates a bot interacting with the airline shopping website
 * with predictable, non-human patterns to trigger the bot detection.
 */
...
const BOT_DELAY = 800; // Delay between actions in ms
...
async function runBot() {
  ...
  // Log this explicitly for debugging
  console.log('Bot headers set:', {
  ...
  try {
    ...
    await page.screenshot({ path: 'bot-initial.png' });
    ...
    try {
      ...
      await page.screenshot({ path: 'bot-results.png' });
      ...
      if (await page.$('button:has-text("Select")')) {
        ...
        await page.screenshot({ path: 'bot-booking.png' });
        ...
        if (await page.$('input[placeholder="Full Name"]')) {
          ...
          if (await page.$('input[placeholder="Card Number"]')) {
            ...
            if (await page.$('button:has-text("Book Flight")')) {
              ...
              await page.screenshot({ path: 'bot-confirmation.png' });
      ...
      await page.screenshot({ path: 'bot-error-form.png' });
    }
    
    // Final waiting and screenshot
    console.log('Waiting to observe bot detection...');
    await sleep(BOT_DELAY * 5);
    await page.screenshot({ path: 'bot-final.png' });
    
  } catch (error) {
    console.error('Bot simulation error:', error);
    await page.screenshot({ path: 'bot-error.png' });
  } finally {
